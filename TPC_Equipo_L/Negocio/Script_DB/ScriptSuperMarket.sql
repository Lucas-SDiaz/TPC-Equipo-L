...
CREATE TABLE Localidades (
  ...
  CONSTRAINT FK_ID_Provincia FOREIGN KEY (ID_Provincia) REFERENCES Provincias (ID)
)
GO
...
Cod_Marca AS 'MARC' + RIGHT('0000' + CONVERT(VARCHAR, ID), 4) PERSISTED PRIMARY KEY NOT NULL,
Nombre_M VARCHAR(50) NOT NULL,
ImgURL_M VARCHAR(1000) NOT NULL,
...
ID INT IDENTITY(1, 1) NOT NULL,
Cod_Categoria AS 'CATE' + RIGHT('0000' + CONVERT(VARCHAR, ID), 4) PERSISTED NOT NULL,
Nombre_C VARCHAR(50) NOT NULL,
ImgURL_C VARCHAR(1000) NOT NULL,
...
ID INT IDENTITY(1, 1) NOT NULL,
Cod_Usuario AS 'USUA' + RIGHT('0000' + CONVERT(VARCHAR, ID), 4) PERSISTED NOT NULL,
NombreUsuario_U NVARCHAR(50) NOT NULL,
Nombre_U NVARCHAR(50) NOT NULL,
Apellido_U NVARCHAR(50) NOT NULL,
Correo_U NVARCHAR(50) UNIQUE NOT NULL,
Contrasenia_U NVARCHAR(50) NOT NULL,
Direccion_U VARCHAR(50) NOT NULL,
Cod_Localidad_U INT NOT NULL,
ImgURL_U VARCHAR(1000),
Estado_U BIT NOT NULL
CONSTRAINT PK_Usuarios PRIMARY KEY(Cod_Usuario),
CONSTRAINT FK_Cod_Localidad_U FOREIGN KEY(Cod_Localidad_U) REFERENCES Localidades(ID)
)
GO
...
Cod_Usuario_V VARCHAR(8) NOT NULL,
Cod_Localidad_V INT NOT NULL,
MontoFinal_V DECIMAL(10, 2) NOT NULL
CONSTRAINT PK_Ventas PRIMARY KEY(Cod_Venta),
CONSTRAINT FK_Cod_Usuario_V FOREIGN KEY(Cod_Usuario_V) REFERENCES Usuarios(Cod_Usuario),
CONSTRAINT FK_Cod_Localidad_V FOREIGN KEY(Cod_Localidad_V) REFERENCES Localidades(ID)
)
GO
...
ID INT IDENTITY(1, 1) NOT NULL,
Cod_Producto AS 'PROD' + RIGHT('0000' + CONVERT(VARCHAR, ID), 4) PERSISTED NOT NULL,
Cod_Categoria_P VARCHAR(8) NOT NULL,
Cod_Marcas_P VARCHAR(8) NOT NULL,
Nombre_P NVARCHAR(50) NOT NULL,
Descripcion_P NVARCHAR(256),
PUnitario_P DECIMAL(10, 2) CHECK(PUnitario_P > 0) NOT NULL,
Stock_P INT NOT NULL,
Estado_P BIT NOT NULL DEFAULT 1
CONSTRAINT PK_Productos PRIMARY KEY(Cod_Producto),
CONSTRAINT FK_Cod_Categoria FOREIGN KEY(Cod_Categoria_P) REFERENCES Categorias(Cod_Categoria)
)
GO

  ----Creación de Tabla Imagenes por Producto
...
GO
  ...
  ----SP para agregar Imagen al Producto
CREATE PROCEDURE spAgregarImagen(
@Cod_Producto VARCHAR(8),
@ImagenURL VARCHAR(1000)
)
AS INSERT INTO ImagenesProd(Cod_Producto, ImagenURL)
VALUES(@Cod_Producto, @ImagenURL)
GO

  ----SP para listar Imagenes del Producto
CREATE PROCEDURE spListarImagenes(
@Cod_Producto VARCHAR(8)
)
AS
BEGIN
	SET NOCOUNT ON;
	SELECT i.ID, i.Cod_Producto, i.ImagenURL
	FROM ImagenesProd AS i
	WHERE i.Cod_Producto = @Cod_Producto
END
GO

  ----SP para actualizar Imagen del producto
CREATE PROCEDURE spActualizarImagen(
@ID INT,
@ImagenURL VARCHAR(1000)
)
AS UPDATE ImagenesProd
SET ImagenURL = @ImagenURL
WHERE ID = @ID
GO

  ----SP para eliminar una Imagen
CREATE PROCEDURE spEliminarImagen(
@ID INT
)
AS DELETE FROM ImagenesProd
WHERE ID = @ID
GO

  ----Creación de Tabla Detalle de ventas
CREATE TABLE Detalle_Ventas(
Cod_Venta_DV INT NOT NULL,
Cod_Producto_DV VARCHAR(8) NOT NULL,
PUnitario_DV DECIMAL(10, 2) NOT NULL,
Cantidad_DV INT NOT NULL
CONSTRAINT PK_Detalle_Ventas PRIMARY KEY(Cod_Venta_DV, Cod_Producto_DV),
CONSTRAINT FK_Cod_Venta_DV FOREIGN KEY(Cod_Venta_DV) REFERENCES Ventas(Cod_Venta),
CONSTRAINT FK_Cod_Producto_DV FOREIGN KEY(Cod_Producto_DV) REFERENCES Productos(Cod_Producto)
)
GO

  ----MARCAS

  ----SP para dar de alta una Marca
CREATE PROCEDURE spAgregarMarca(
@Nombre_M VARCHAR(50),
@ImgURL_M VARCHAR(1000),
@Estado_M BIT
)
AS INSERT INTO Marcas(Nombre_M, ImgURL_M, Estado_M)
VALUES (@Nombre_M, @ImgURL_M, @Estado_M)
GO

  ----SP para listar una Marca
CREATE PROCEDURE spListarMarca
AS
BEGIN
	SET NOCOUNT ON;
	SELECT mar.Cod_Marca, mar.Nombre_M, mar.ImgURL_M
	FROM Marcas AS mar
	WHERE mar.Estado_M = 1
END
GO

  ----SP para actualizar una Marca
...
AS UPDATE Marcas
SET Nombre_M = @Nombre_M, ImgURL_M = @ImgURL_M, Estado_M = @Estado_M
WHERE Cod_Marca = @Cod_Marca
GO

  ----SP para eliminar una Marca
CREATE PROCEDURE spEliminarMarca(
@Cod_Marca VARCHAR(8)
)
AS DELETE FROM Marcas WHERE Cod_Marca = @Cod_Marca
GO

  ----CATEGORIAS

  ----SP para dar de alta una Categoria
CREATE PROCEDURE spAgregarCategoria(
@Nombre_C VARCHAR(50),
@ImgURL_C VARCHAR(1000),
@Estado_C BIT
)
AS INSERT INTO Categorias(Nombre_C, ImgURL_C, Estado_C) 
VALUES (@Nombre_C, @ImgURL_C, @Estado_C)
GO
  ----SP para listar una Categoria
CREATE PROCEDURE spListarCategorias
AS
BEGIN
	SET NOCOUNT ON;
	SELECT cat.Cod_Categoria, cat.Nombre_C, cat.ImgURL_C
	FROM Categorias AS cat
	WHERE cat.Estado_C = 1
END
GO
  ----SP para actualizar una Categoria
CREATE PROCEDURE spActualizarCategoria(
@Cod_Categoria VARCHAR(8),
@Nombre_C VARCHAR(50),
@ImgURL_C VARCHAR(1000),
@Estado_C BIT
) 
AS UPDATE Categorias 
SET Nombre_C = @Nombre_C, ImgURL_C = @ImgURL_C, Estado_C = @Estado_C
WHERE Cod_Categoria = @Cod_Categoria
GO

  ----SP para eliminar una Categoria
CREATE PROCEDURE spEliminarCategoria(
@Cod_Categoria VARCHAR(8)
) 
AS DELETE FROM Categorias WHERE Cod_Categoria = @Cod_Categoria
GO
...
@Cod_Categoria_P VARCHAR(8),
@Nombre_P NVARCHAR(50),
@Descripcion_P NVARCHAR(256),
@PUnitario_P DECIMAL(10, 2),
@Stock_P INT,
@Estado_P BIT
) 
AS INSERT INTO Productos(Cod_Categoria_P, Nombre_P, Descripcion_P, PUnitario_P, Stock_P, Estado_P) 
VALUES(@Cod_Categoria_P, @Nombre_P, @Descripcion_P, @PUnitario_P, @Stock_P, @Estado_P)
GO

  ----SP para listar un Producto
CREATE PROCEDURE spListarProductos
AS
BEGIN 
	SET NOCOUNT ON;
	SELECT pro.Cod_Producto, pro.Cod_Categoria_P, pro.Nombre_P, pro.Descripcion_P, pro.PUnitario_P, pro.Stock_P
	FROM Productos AS pro
	INNER JOIN Categorias AS cat ON pro.Cod_Categoria_P = cat.Cod_Categoria
	WHERE pro.Estado_P = 1
END
GO

  ----SP para listar Productos por Categoria o Marcas
CREATE PROCEDURE spListarProductosCoM(
@Cod_Categoria_P VARCHAR(8),
@Cod_Marca_P VARCHAR(8)
)
AS
BEGIN 
	SET NOCOUNT ON;
	SELECT pro.Cod_Producto, pro.Cod_Categoria_P, Pro.Cod_Marcas_P, pro.Nombre_P, pro.Descripcion_P, pro.PUnitario_P, pro.Stock_P
	FROM Productos AS pro 
	WHERE pro.Estado_P = 1 AND pro.Cod_Categoria_P = @Cod_Categoria_P OR pro.Cod_Marcas_P = @Cod_Marca_P
END
GO
...
AS UPDATE Productos 
SET Cod_Categoria_P = @Cod_Categoria_P, Nombre_P = @Nombre_P, Descripcion_P = @Descripcion_P,
	PUnitario_P = @PUnitario_P, Stock_P = @Stock_P, Estado_P = @Estado_P
...
AS DELETE FROM Productos WHERE Cod_Producto = @Cod_Producto
GO
...
@NombreUsuario_U VARCHAR(50),
@Nombre_U VARCHAR(50),
@Apellido_U NVARCHAR(50),
@Correo_U NVARCHAR(50),
@Contrasenia_U NVARCHAR(50) ,
@Direccion_U VARCHAR(50),
@Cod_Localidad_U INT,
@ImgURL_U VARCHAR(1000),
@Estado_U BIT
)
AS INSERT INTO Usuarios(NombreUsuario_U, Nombre_U, Apellido_U, Correo_U, Contrasenia_U, Direccion_U, Cod_Localidad_U, ImgURL_U, Estado_U)
VALUES(@NombreUsuario_U, @Nombre_U, @Apellido_U, @Correo_U, @Contrasenia_U, @Direccion_U, @Cod_Localidad_U, @ImgURL_U, @Estado_U)
GO
...
BEGIN 
	SET NOCOUNT ON;
	SELECT u.Cod_Usuario, u.NombreUsuario_U, u.Nombre_U, u.Apellido_U, u.Correo_U, u.Contrasenia_U, u.Direccion_U, u.Cod_Localidad_U, u.ImgURL_U
	FROM Usuarios AS u
	WHERE u.Estado_U = 1
END
GO
...
AS UPDATE Usuarios
SET NombreUsuario_U = @NombreUsuario_U, Nombre_U = @Nombre_U, Apellido_U = @Apellido_U, Correo_U = @Correo_U, Contrasenia_U = @Contrasenia_U,
	Direccion_U = @Direccion_U, Cod_Localidad_U = @Cod_Localidad_U, ImgURL_U = @ImgURL_U, Estado_U = @Estado_U
...
AS DELETE FROM Usuarios WHERE Cod_Usuario = @Cod_Usuario
GO

  ----Trigger para actualizar Stock Productos
CREATE TRIGGER tgDetalleVenta ON Detalle_Ventas
FOR INSERT
AS
DECLARE @Cantidad INT;
DECLARE @Cod_Producto VARCHAR(8);
DECLARE @Stock_P INT;
SELECT @Cantidad = Cantidad_DV FROM inserted;
SELECT @Cod_Producto = Cod_Producto_DV FROM inserted;
SET @Stock_P = (SELECT @Stock_P FROM Productos WHERE Cod_Producto = @Cod_Producto);
IF(@Cantidad > 0 AND @Cantidad < 10)
BEGIN
IF(@Stock_P <= 2)
BEGIN
RAISERROR ('Stock Insuficiente', 15, 1);
ROLLBACK TRANSACTION --Anula Evento INSERT
END
ELSE
BEGIN
 ----Actualizo la tabla Producto del pivote Stock y reducimos.
UPDATE Productos SET Stock_P=Stock_P-@Cantidad WHERE Cod_Producto = @Cod_Producto
END
END
ELSE
BEGIN
RAISERROR('La Cantidad no es correcta, tiene que ser del 1 al 9', 15, 1);
ROLLBACK TRANSACTION --Anula Evento INSERT
END
GO
